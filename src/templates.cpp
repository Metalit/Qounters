...
std::vector<std::string_view> const NotesDisplayStrings = {
    "Cut",
    "Remaining",
    "Cut Ratio",
    "Cut Percent",
...
std::vector<std::string_view> const SongTimeDisplayStrings = {
    "Base Game",
    "Ring",
    "Percentage",
};
...
namespace Qounters::Templates {
    ...
    void CreateButtons(UnityEngine::GameObject* parent, std::function<void()> createFn) {
        ...
        buttons->spacing = 3;
        BSML::Lite::CreateUIButton(buttons, "Cancel", CloseModal);
        BSML::Lite::CreateUIButton(buttons, "Create", "ActionButton", [createFn]() {
            createFn();
            CloseModal();
        });
    }
    ...
    Component&
    AddText(Group& group, std::string source, UnparsedJSON sourceOpts, float size = 15, TextOptions::Aligns align = TextOptions::Aligns::Center) {
    ...
    void AddAverageCut(int anchor, UnityEngine::Vector2 pos, bool splitSaber, bool splitCut, int decimals) {
        ...
        auto addLine = [&splitSaber, &group](TextSource::AverageCut opts, float yPos) {
            if (splitSaber) {
                ...
                AddText(group, TextSource::AverageCutName, opts, 15, TextOptions::Aligns::Right).Position = UnityEngine::Vector2(-2, yPos);
                opts.Saber = (int) Sabers::Right;
                AddText(group, TextSource::AverageCutName, opts, 15, TextOptions::Aligns::Left).Position = UnityEngine::Vector2(2, yPos);
            } else
    ...
    void ScoreUI(UnityEngine::GameObject* parent) {
        ...
        Utils::CreateDropdownEnum(parent, "Starting Anchor", anchor, AnchorStrings, [](int val) { anchor = val; });
        BSML::Lite::CreateToggle(parent, "Show Numeric Score", score, [](bool val) { score = val; });
    ...
    void PPUI(UnityEngine::GameObject* parent) {
        ...
        Utils::CreateDropdownEnum(parent, "Starting Anchor", anchor, AnchorStrings, [](int val) { anchor = val; });
        BSML::Lite::CreateToggle(parent, "Show ScoreSaber", scoresaber, [](bool val) { scoresaber = val; });
