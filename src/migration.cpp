...
namespace Qounters::Migration {
    std::pair<int, UnityEngine::Vector2> EstimatePosition(std::string position, float distance, OldConfig const& config) {
        ...
        if (position == "BelowCombo") {
            ...
            offset += 27;
            offset -= 25;
            return {(int) Group::Anchors::Right, {0, -distance + offset}};
            ...
            float offset = -13;
            offset += 25;
            return {(int) Group::Anchors::Bottom, {0, -distance + offset}};
        } else if (position == "AboveHighway") {
            float offset = 7;
            offset += 25;
            return {(int) Group::Anchors::Top, {0, distance + offset}};
        }
        return {(int) Group::Anchors::Left, {0, 0}};
    }
    ...
    void Migrate() {
        ...
        auto newPreset = GetDefaultHUDPreset();
        ...
        if (config.NotesLeftConfig.Enabled) {
            ...
            Templates::AddNotes(anchor, pos, (int) TextSource::Notes::Displays::Remaining, 0);
        }
        ...
        if (config.NoteConfig.Enabled) {
            ...
            int display = (int) (config.NoteConfig.ShowPercentage ? TextSource::Notes::Displays::Percent : TextSource::Notes::Displays::Ratio);
        ...
        if (config.ItalicText) {
            for (auto& group : newPreset.Qounters) {
                for (auto& comp : group.Components) {
                    if (comp.Type == (int) Component::Types::Text) {
                        auto opts = *comp.Options.GetValue<TextOptions>();
                        opts.Italic = true;
                        comp.Options = opts;
                    }
                }
